...
if grep '^[[:digit:]]*$' <<< "$TeamID";then 
	...
	$CMDSH AdminWarnById $PlayerID TeamID未使用整数型数字。
	exit
fi 
if grep '^[[:digit:]]*$' <<< "$SquadID";then 
	echo "$SquadID is OK." 
else 
	$CMDSH AdminWarnById $PlayerID SquadID未使用整数型数字。
	exit
fi 
if grep '^[[:digit:]]*$' <<< "$Hour";then 
	echo "$Hour is OK." 
else 
	$CMDSH AdminWarnById $PlayerID 发放的小时数未使用整数型数字。
	exit
fi 
if [ -z $Hour ]; then
	$CMDSH AdminWarnById $PlayerID 未填写发放的小时数。
	exit
fi 


#获取玩家64IDTeam ID: 2 | Squad ID: 5 
PLAYER=(`cat $ListPlayer |grep "Team ID: $TeamID | Squad ID: $SquadID "  |awk '{print $9}'`)
...
do
	...
	if [ $COUNTER -eq $PLAYERSUM ];then
		$CMDSH AdminBroadcast 系统已完成对阵营${TeamID}，小队${SquadID}，${PLAYERSUM}名玩家，发放${days}${unit}小时预留位，发送“info”可查预留位到期时间。
		sleep 1
		break
	fi
done
