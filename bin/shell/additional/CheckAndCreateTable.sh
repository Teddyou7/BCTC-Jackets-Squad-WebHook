#!/bin/bash
#初始化数据库
#CheckAndCreateTable.sh

source $(echo $(cd `dirname $0`; pwd) | sed 's/bin\/shell\/additional//g')/cfg/config.sh

mysql -h $db_host -P $db_port -u $db_user -D $db_name <<EOF

-- 检查并创建 bctc6_combat_info 表
CREATE TABLE IF NOT EXISTS ${db_server}combat_info (
...
-- 检查并创建 ${db_server}historical_layers 表
CREATE TABLE IF NOT EXISTS ${db_server}historical_layers (
    id INT AUTO_INCREMENT PRIMARY KEY COMMENT 'ID',
    map_name VARCHAR(255) NOT NULL COMMENT '地图名称',
    timestamp DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录时间'
) COMMENT='历史图层表';

-- 检查并创建 ${db_server}squad_creation_info 表
CREATE TABLE IF NOT EXISTS ${db_server}squad_creation_info (
    id INT AUTO_INCREMENT PRIMARY KEY COMMENT 'ID',
...
-- 检查并创建 ${db_server}player_stats 表
CREATE TABLE IF NOT EXISTS ${db_server}player_stats (
    id INT AUTO_INCREMENT PRIMARY KEY COMMENT 'ID',
...
-- 当前的对局战斗信息
CREATE OR REPLACE VIEW ${db_server}combat_info_0 AS
SELECT ci.*
FROM ${db_server}combat_info ci
JOIN (
    SELECT MAX(timestamp) AS latest_map_time
    FROM ${db_server}historical_layers
) hl ON ci.action_time >= hl.latest_map_time;

-- 上局的对局战斗信息
CREATE OR REPLACE VIEW ${db_server}combat_info_1 AS
SELECT ci.*
FROM ${db_server}combat_info ci
JOIN (
    SELECT 
...
-- 上上局的对局战斗信息
CREATE OR REPLACE VIEW ${db_server}combat_info_2 AS
SELECT ci.*
FROM ${db_server}combat_info ci
JOIN (
    SELECT 
