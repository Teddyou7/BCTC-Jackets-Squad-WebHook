...
# 连接数据库并查询最新余额
current_balance=$(mysql -h "$db_host" -u "$db_user" -p"$db_password" -D "$db_name" -s -N -e "SELECT current_balance FROM transactions WHERE steamid='$steamid' AND organization_tag='$organization_tag' ORDER BY transaction_time DESC LIMIT 1;")
...
# 向数据库写入新记录
mysql -h "$db_host" -u "$db_user" -p"$db_password" -D "$db_name" -e "INSERT INTO transactions (steamid, organization_tag, transaction_amount, current_balance, type, note) VALUES ('$steamid', '$organization_tag', '$amount', '$new_balance', IF('$operation' = 'add', 1, 0), '$note');"
